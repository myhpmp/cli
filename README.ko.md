# claude-hp-mp

한국어 | [English](./README.md)

[Claude Code](https://docs.anthropic.com/en/docs/claude-code) 사용량을 RPG 게임처럼 보여주는 대시보드.

Claude Code 사용을 게임으로 만들어보세요 — 레벨, HP(세션 한도), MP(주간 한도), EXP, 연속 사용일수를 추적하고 칭호를 획득하세요.

## 상태 바

Claude Code 하단에 항상 표시:

```
⚔️ 견습 전사 Lv.9 ★★★ | ❤️ 43% | 💙 76% | 🧠 25% | 🔥7일
```

## 상세 보기

`claude-hp-mp usage`로 전체 대시보드 확인:

```
🎮 ⚔️ 견습 전사 Lv.9 ★★★                    🔥 연속: 7일
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❤️ HP  ████░░░░░░   43%  ⏱️ 2h30m
💙 MP  ████████░░   76%
🧠 CTX ███░░░░░░░   25%  (250K / 1.0M context)
⭐ EXP ██████░░░░   62%  (186 / 300 → Lv.10)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 총 누적 EXP: 2,886  |  총 세션: 47 회
```

## 스탯

| 스탯 | 설명 |
|------|------|
| **❤️ HP** | 5시간 세션 한도 잔여량 (%) + 리셋 시간 |
| **💙 MP** | 7일 주간 한도 잔여량 (%) |
| **🧠 CTX** | 현재 컨텍스트 윈도우 사용률 (%) |
| **🔥 연속** | 연속 사용일수 |
| **⭐ EXP** | 다음 레벨까지 경험치 |

## 레벨 티어

| 레벨 | 칭호 | 레벨당 EXP | 누적 EXP |
|------|------|-----------|----------|
| 1-5 | 🌱 초보 모험가 | 100 | 500 |
| 6-10 | ⚔️ 견습 전사 | 300 | 2,000 |
| 11-15 | 🛡️ 숙련 기사 | 600 | 5,000 |
| 16-20 | 🧙 마법사 | 1,200 | 11,000 |
| 21-30 | 🔮 대마법사 | 3,500 | 46,000 |
| 31-40 | 👑 아크메이지 | 8,000 | 126,000 |
| 41-50 | 🐉 전설의 코드드래곤 | 15,000 | 276,000 |
| 50+ | ⚡ 초월자 | 25,000 | ∞ |

초반 레벨은 빠르게 올라갑니다. 후반 티어는 꾸준한 사용이 필요합니다 — ⚡ 초월자까지 약 8개월 소요.

## EXP 획득 방식

| 행동 | EXP |
|------|-----|
| 토큰 사용 | 1K 토큰당 1 EXP |
| 세션 완료 | 25 EXP |
| 연속 사용 보너스 | 연속일수 × 5 EXP (최대 30일 = 150 EXP) |
| 주간 70%+ 사용 | 100 EXP |

## 빠른 시작

```bash
# Claude Code 자동 설정 (hooks + 상태 바)
npx claude-hp-mp setup

# 표시 언어 설정
npx claude-hp-mp locale

# Claude Code를 재시작하면 상태 바가 나타납니다
```

끝! 상태 바가 자동으로 표시되고, hook을 통해 EXP가 쌓이기 시작합니다.

## 명령어

| 명령어 | 설명 |
|--------|------|
| `npx claude-hp-mp setup` | Claude Code hook과 상태 바 자동 설정 |
| `npx claude-hp-mp usage` | RPG 대시보드 상세 보기 |
| `npx claude-hp-mp sync` | 수동으로 클라우드 동기화 |
| `npx claude-hp-mp statusline` | 상태 바 켜기/끄기 토글 |
| `npx claude-hp-mp statusline on` | 상태 바 켜기 |
| `npx claude-hp-mp statusline off` | 상태 바 끄기 |
| `npx claude-hp-mp locale` | 표시 언어 변경 (한국어/English) |
| `npx claude-hp-mp init` | 인증 설정 (크로스 디바이스 동기화) |

## 크로스 디바이스 동기화

여러 기기에서 스탯(레벨, EXP, 연속일수)을 동기화:

```bash
npx claude-hp-mp init
```

**GitHub OAuth**와 **Google OAuth** 인증을 지원합니다.

### 동기화 방식

| 시점 | 동작 |
|------|------|
| 세션 시작 | 클라우드에서 최신 데이터 가져오기 |
| 5분마다 | 사용 중 자동 동기화 |
| 세션 종료 | 최종 스탯 클라우드에 저장 |
| `npx claude-hp-mp sync` | 수동 즉시 동기화 |

데이터는 `~/.claude-hp-mp/data.json`에 로컬 저장되며 오프라인에서도 동작합니다. 클라우드 동기화에 실패하면 로컬 데이터가 보존되고, 다음 기회에 동기화됩니다.

## 요구 사항

- Node.js >= 18
- Claude Code 구독 (Pro/Max)
- OAuth 로그인 (`~/.claude/.credentials.json` 사용)

## 지원 플랫폼

- **Windows**
- **macOS**
- **Linux**

모든 플랫폼에서 동일한 크레덴셜 경로(`~/.claude/.credentials.json`)를 사용합니다.

## 다국어

한국어와 영어를 지원합니다. `npx claude-hp-mp locale`로 언어를 설정하거나, 시스템 로캘에서 자동 감지합니다.

```
KO: ⚔️ 견습 전사 Lv.9 ★★★ | ❤️ 43% | 💙 76% | 🧠 25% | 🔥7일
EN: ⚔️ Apprentice Warrior Lv.9 ★★★ | ❤️ 43% | 💙 76% | 🧠 25% | 🔥7d
```

## 작동 방식

1. **상태 바** — Claude Code가 세션 JSON(rate limit, 컨텍스트 사용량)을 상태 바 스크립트에 전달 → RPG HUD로 렌더링
2. **Hook** — `PostToolUse`가 토큰 사용량 추적 → EXP 변환. `SessionStart`/`Stop`이 세션과 연속일수 추적
3. **로컬 저장소** — 모든 데이터가 `~/.claude-hp-mp/data.json`에 저장 (오프라인 동작)
4. **클라우드 동기화** — Supabase 연동, 5분 간격 + 세션 경계에서 자동 동기화. Last-write-wins 충돌 해결

## 라이선스

MIT
